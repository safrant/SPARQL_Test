...
SPARQL_ENDPOINT="https://sparql-evs.nci.nih.gov/ctrp"
GRAPH="<http://NCIt>"
...
def listVocabsInGraph(GRAPH):
    print("List of vocabs in graph")
    QUERY='''select * from $GRAPH
    where {?s ?p ?o .
    ?s a owl:Ontology} limit 20'''
...
#     GRAPH="<http://ncicb.nci.nih.gov/owl/EVS/Thesaurus.owl>"
    ...
    T0 = time()
    results = listVocabsInGraph(GRAPH)
    T1 = time()
    DIFF = (T1 - T0)
    print(DIFF)
